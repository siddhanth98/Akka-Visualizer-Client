...
public class Actor2 extends AbstractBehavior<Actor2.Command> {
    ...
    public Receive<Actor2.Command> createReceive() {
        return newReceiveBuilder()
                .onMessage(Actor2.Increment.class, m -> this.onIncrement())
                .onMessage(Actor2.Display.class, m -> this.display())
                .onMessage(Actor2.PingActor.class, m -> this.pingActor(m.replyTo))
                .onMessage(Actor2.State.class, m -> this.syncState())
                .onMessage(Actor2.Greeting.class, m -> this.getGreet(m.message))
    ...
    private Behavior<Actor2.Command> onIncrement() { /* message received, let vis know */
        this.count++;
        return this;
    }

    private Behavior<Actor2.Command> display() {
        getContext().getLog().info(String.format("My current count - %d%n", this.count));
        return this;
    }

    private Behavior<Actor2.Command> pingActor(ActorRef<Actor2.Command> replyTo) {
        Display msg = (Actor2.Display) this.vis.new MessageWrapper(new Actor2.Display(this.key), "send", replyTo.path().toString()).getMessage();
    ...
    private Behavior<Actor2.Command> getGreet(String message) {
        getContext().getLog().info(message);
        return this;
    }

    public Behavior<Actor2.Command> syncState() {
